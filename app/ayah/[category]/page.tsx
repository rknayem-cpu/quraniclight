...
// টাইপস ডিফাইন করা হলো
interface CategoryTitles {
  [key: string]: string;
}

interface Post {
  _id: { toString(): string }; // MongoDB ObjectId handling
  title: string;
}
...
async function CategoryListPage({ params }: CategoryListPageProps) {
  ...
  try {
    await connectDB();
    // MongoDB ডাটাবেস থেকে ডাটা ফেচ করা
    posts = await Ayah.find({ category: currentCategory })
      .select("title")
      .sort({ createdAt: -1 })
      .lean(); // .lean() ব্যবহার করলে ডাটা সাধারণ অবজেক্ট হিসেবে আসে যা টাইপস্ক্রিপ্টের জন্য ভালো
  } catch (error) {
        ...
        <div className="space-y-4">
          {posts.map((post, index) => (
            <Link
              ...
              href={`/ayah/${currentCategory}/${post._id.toString()}`}
