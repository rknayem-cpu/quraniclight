...
import { FaCopy, FaCheck, FaInfoCircle, FaHandHoldingHeart } from 'react-icons/fa';
...
const bkashAccounts: BkashAccount[] = [
  { type: 'পার্সোনাল (Personal)', number: '01321302589', label: 'Send Money করুন' },
];

function SupportPage() {
  // useState-এ টাইপ সেট করা (null অথবা string হতে পারে)
      ...
      <div className="relative max-w-2xl w-full bg-white border border-emerald-100/80 p-8 md:p-14 rounded-[3rem] shadow-[0_20px_60px_rgba(4,36,20,0.03)] z-10 flex flex-col items-center">
        ...
        <div className="bg-slate-50/80 border border-slate-100 p-6 md:p-8 rounded-2xl text-center mb-10 relative">
          <p className="text-slate-950 text-lg md:text-xl font-bold leading-relaxed">
            "দ্বীনের এই মহৎ কাজে যদি আপনি আমাদের পাশে থাকতে চান বা সহযোগিতা করতে চান, তবে আপনার সামর্থ্য অনুযায়ী কিছু আর্থিক সহযোগিতা আমরা আন্তরিকভাবে কামনা করি।"
          </p>
        </div>
        ...
        <p className="text-sm font-bold text-emerald-800 italic text-center mb-8 bg-emerald-50/50 px-4 py-2 rounded-xl border border-emerald-100/40">
          "নিশ্চয়ই দান সম্পদ কমিয়ে দেয় না।" — সহীহ মুসলিম, হাদীস ২৫৮৮
        </p>
        ...
        <div className="w-full space-y-5">
          ...
          {bkashAccounts.map((account) => (
            <div 
              key={account.type}
              className="group relative bg-white border border-slate-100 hover:border-pink-200/60 p-5 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 transition-all duration-300"
            >
              <div className="flex items-center gap-4 w-full sm:w-auto">
                ...
                <div>
                  <h3 className="text-slate-950 font-black text-base md:text-lg">{account.type}</h3>
                ...
                <button
                  ...
                  className={`p-2.5 rounded-lg transition-all active:scale-95 border ${
                    copiedType === account.type 
                      ...
                      : 'bg-white border-slate-200 text-slate-800 hover:bg-pink-50 hover:text-[#E2136E]'
                  }`}
