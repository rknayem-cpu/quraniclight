...
// সাব-পোস্টের জন্য টাইপ ডিফাইন
interface SubPostFormData {
  title: string;
  content: string;
  category: string;
  imgUrl: string;
...
function EditSubPostPage() {
  const { id } = useParams();
  const router = useRouter();
  const [loading, setLoading] = useState<boolean>(true);
  
  const [formData, setFormData] = useState<SubPostFormData>({
    title: '',
    content: '',
    category: 'halal-haram', // ডিফল্ট ভ্যালু
    imgUrl: '',
    note: ''
  });
  ...
  useEffect(() => {
    if (!id) return;
    axios.get(`/api/sub-posts/${id}`).then((res) => {
      setFormData(res.data);
      setLoading(false);
    });
  }, [id]);

  // জেনেরিক হ্যান্ডলার (সব ইনপুটের জন্য)
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    try {
      await axios.put(`/api/sub-posts/${id}`, formData);
      alert("আপডেট সফল হয়েছে!");
      router.push('/admin/sub-posts');
    } catch (error) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 font-sans">
      <Link href="/admin/sub-posts" className="flex items-center gap-2 text-slate-500 font-bold mb-6 hover:text-emerald-600">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
        <h2 className="text-2xl font-black text-slate-900 mb-6">সাব-পোস্ট সম্পাদনা করুন</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <select name="category" className="w-full p-4 border border-slate-200 rounded-2xl outline-none" value={formData.category} onChange={handleChange}>
              {['halal-haram', 'quranic-laws', 'hudud', 'social-life', 'business', 'morality', 'fitna'].map(cat => (
...
export default EditSubPostPage;